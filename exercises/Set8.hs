...
combine blender (Picture f1) (Picture f2) = Picture $ blender <$> f1 <*> f2
...
rectangle x0 y0 w h = Shape $ \(Coord x y) -> x >= x0 && x < (x0 + w)
                                           && y >= y0 && y < (y0 + h)
------------------------------------------------------------------------------
...
union (Shape f1) (Shape f2) = Shape $ (||) <$> f1 <*> f2

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape $ (&&) <$> f1 <*> (not <$> f2)
------------------------------------------------------------------------------
...
paint (Picture pat) (Shape shape) (Picture base) = Picture $ \c ->
  if shape c
    then pat c
    else base c
------------------------------------------------------------------------------
...
instance (Transform a, Transform b) => Transform (Chain a b) where
  apply (Chain f1 f2) = apply f1 . apply f2
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply _ (Picture f) = Picture
    $ \(Coord x y) -> divColor 5 $ foldr1 sumColor $ map (f . uncurry Coord) (neighbours x y)
      where
        neighbours x y = [(x-1, y), (x,y), (x+1, y), (x, y-1), (x, y+1)]
        sumColor (Color r1 g1 b1) (Color r2 g2 b2) = Color (r1 + r2) (g1 + g2) (b1 + b2)
        divColor n (Color r1 g1 b1) = Color (r1 `div` n) (g1 `div` n) (b1 `div` n)
------------------------------------------------------------------------------
...
instance Transform BlurMany where
  apply (BlurMany n) = foldr1 (.) $ replicate n $ apply Blur
------------------------------------------------------------------------------
