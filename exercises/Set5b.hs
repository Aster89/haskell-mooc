...
allValues condition (Node v l r) = condition v
                                && allValues condition l
...
isOrdered (Node v l r) = allValues (< v) l
                      && allValues (> v) r
                      && isOrdered l
                      && isOrdered r
...
set (StepL:path) val (Node v l r) = Node v (set path val l) r
set (StepR:path) val (Node v l r) = Node v l (set path val r)
...
search v (Node v' l r)
  | v == v' = Just []
  | otherwise = ((StepL:) <$> search v l) `mplus` ((StepR:) <$> search v r)
  where
    j@(Just _) `mplus` _ = j
    _ `mplus` m = m
