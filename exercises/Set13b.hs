...
visit maze place = do
  visited <- get
  when (not $ place `elem` visited) $ do
    modify $ (place:)
    mapM_ (visit maze) (fromJust $ lookup place maze)
    where
      fromJust (Just x) = x
      fromJust Nothing = error "d'oh!"
...
path maze place1 place2 = let reachableFrom = runState . visit maze
                          in place2 `elem` snd (reachableFrom place1 [])
...
instance Monad SL where
  -- implement return and >>=
  return x = SL $ \s -> (x, s, [])
  SL sl >>= f = SL $ \s -> let (a, s1, l1) = sl s
                               (b, s2, l2) = runSL (f a) s1
                           in (b, s2, l1 ++ l2)
