...
-- Database
import Database.SQLite.Simple (open,execute,execute_,query,query_,Connection,Query(..), Only (..), fromOnly)
...
balance db account = do
  ls <- query db balanceQuery (Only account) :: IO [(Only Int)]
  return $ sum $ fmap fromOnly ls
...
parseCommand (operation:recipient:amount:[])
  = parseInt amount
    >>= case T.unpack operation of
          "deposit" -> Just . Deposit recipient
          "withdraw" -> Just . Deposit recipient . negate
          _ -> const Nothing
parseCommand (b:r:[]) = case T.unpack b of
                          "balance" -> Just $ Balance r
                          _ -> Nothing
...
perform db (Just (Deposit r a)) = deposit db r a >> return (T.pack "OK")
perform db (Just (Balance r)) = balance db r >>= return . T.pack . show
...
server db request respond = do
  print request
  parsePathAndPerform request >>= respond'
  where
    parsePathAndPerform = perform db . parseCommand . pathInfo
    respond' = respond . responseLBS status200 [] . encodeResponse
