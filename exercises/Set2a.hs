...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = let r = drop n xs
                 in if null r
                      then xs
                      else drop (length r) xs
...
palindromify s = let n = length s
                     (lhalf, rhalf) = splitAt (n `div` 2) s
                     go "" c _ = c
                     go _ c "" = c
                     go (l:ls) c (r:rs)
                      | l == r = go ls (l:c ++ [r]) rs
                      | otherwise = c
                     (c, rhalf') = splitAt (if even n then 0 else 1) rhalf
                 in go (reverse lhalf) c rhalf'
