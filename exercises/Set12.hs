...
instance Functor TwoList where
  ...
  fmap f (TwoNode a b cs) = TwoNode (f a) (f b) $ fmap f cs
...
count x = foldl' go 0
  where
    go acc e = if e == x then (acc + 1) else acc
...
inBoth as bs = filter (`elem` bs) $ foldr (:) [] as
...
instance Foldable List where
  foldr step zero Empty = zero
  foldr step zero (LNode a rest) = step a $ foldr step zero rest
...
instance Foldable TwoList where
  foldr step zero TwoEmpty = zero
  foldr step zero (TwoNode a b cs) = step a $ step b $ foldr step zero cs
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node v l r) = sumTree l `mappend` v `mappend` sumTree r
