...
greetText text = let (first15, rest) = T.splitAt 15 text
                     dots = T.pack $ if T.null rest then "" else "..."
                 in (T.pack "Hello, ") <> first15 <> dots <> T.pack "!"
...
shout :: T.Text -> T.Text
shout text = T.unwords $ go $ T.words text
  where
    go [] = []
    go (a:b:cs) = (T.toUpper a):b:go cs
    go (a:cs) = (T.toUpper a):cs
...
longestRepeat text = case (map T.length $ T.group text) of
                      [] -> 0
                      a -> maximum a
...
byteRange s
  | B.null s = 0
  | otherwise = let str = B.unpack s in maximum str - minimum str
...
xorChecksum bs
  | B.null bs = 0
  | otherwise = foldr1 xor $ B.unpack bs
...
countUtf8Chars t = case decodeUtf8' t of
                     Left _ -> Nothing
...
pingpong b = let lb = B.fromStrict b
                 bl = B.fromStrict $ B.reverse b
             in lb `mappend` bl `mappend` pingpong b
