...
oddFactorial n = helper $ n - (if even n then 1 else 0)
  where
    helper 1 = 1
    helper n = n * (oddFactorial $ n - 2)
...
countdown n = "Ready! " ++  (reverse [1..n] >>= (++ "... ") . show) ++ "Liftoff!"
...
smallestDivisor 0 = error "not meant to work"
smallestDivisor 1 = error "not meant to work"
smallestDivisor n = helper n 2
  where
    helper n d = if n `mod` d == 0 then d else helper n (d + 1)
...
biggestPrimeAtMost 0 = error "not meant to work"
biggestPrimeAtMost 1 = error "not meant to work"
