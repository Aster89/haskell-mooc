...
simplify (RationalNumber n d) = let gd = gcd n d
                                in RationalNumber (n `div` gd) (d `div` gd)
...
class Cycle a where
  step :: a -> a
  stepMany :: Int -> a -> a
  stepMany n = head . drop n . iterate step
