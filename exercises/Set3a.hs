...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if ((>) `on` measure) a b then a else b
  where
...
capitalize = unwords . (mapHead toUpper <$>) . words
  where
    mapHead f (a:as) = f a:as
    mapHead f _ = error "not meant to be called"
...
while check update value = head $ dropWhile check $ iterate update value
...
whileRight check x = getLeft
  ...
  where
    ...
    isRight _ = False
    getLeft (Left a) = a
    getLeft _ = error "not meant to be called"
...
joinToLength n strings = [a ++ b | a <- strings, b <- strings, length a + length b == n]
...
sumRights = sum . (>>= (either (const []) pure))
...
interpreter commands = go (0, 0) commands
  where
    ...
    go xy@(x, _) ("printX":cs) = show x:go xy cs
    go xy@(_, y) ("printY":cs) = show y:go xy cs
    go xy ("q":cs) = []
    go xy (_:cs) = go xy cs
    go xy [] = []
