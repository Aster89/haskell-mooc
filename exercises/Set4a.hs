...
allEqual [] = True
allEqual [_] = True
allEqual (x0:xs@(x1:_)) = x0 == x1 && allEqual xs
...
rangeOf [] = error "not meant to work"
rangeOf [x] = x
rangeOf (x:xs) = go x x xs
  where
    go mn mx [] = mx - mn
    go mn mx (x:xs) = go (min mn x) (max x mx) xs
...
longest :: Ord a => [[a]] -> [a]
longest [] = error "not meant to work"
longest [l] = l
longest (l0@(h0:_):l1@(h1:_):ls) = longest
  $ case length l0 `compare` length l1 of
    GT -> (l0:ls)
    LT -> (l1:ls)
    EQ -> if h0 < h1 then (l0:ls) else (l1:ls)
...
incrementKey k [] = []
incrementKey k ((k', v'):rest) = (k', if k == k' then v' + 1 else v'):incrementKey k rest
...
average [] = error "not meant to work"
...
winner scores player1 player2 = maximumBy (compare `on` points) [player2, player1]
  where
    points p = Map.findWithDefault 0 p scores
    on :: (b -> b -> c) -> (a -> b) -> a -> a -> c
...
transfer from to amount bank = case (amount > 0, Map.lookup from bank, Map.member to bank) of
                                 (True, Just available, True)
                                    | available < amount
                                      -> bank
                                    | otherwise
                                      -> Map.alter (fmap (+ amount)) to
                                       $ Map.alter (fmap (subtract amount)) from bank
                                 (_, _, _) -> bank
...
maxIndex arr = fst $ maximumBy (compare `on` snd) (assocs arr)
    where
      on :: (b -> b -> c) -> (a -> b) -> a -> a -> c
      f `on` g = \x y -> f (g x) (g y)
